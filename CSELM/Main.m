...
%								    %
%                                                                   %
%  programmers: Fatemeh Bandi, Somayeh Bassiri                      %
%								    %
%								    %
%        paper: 					            %
...
global NumberofHiddenNeurons
  name_data=xlsread('bupa');
%   name_data=load('diabetes1.txt');
    size_train=199;

data=name_data;
 data=mapminmax(data',-1,1)';
[size_data, size_Features]=size(data);
index=randperm(size_data);


d_train=data(index(1:size_train),:);
d_test=data(index(1+size_train:end),:);

size_Features=size_Features-1;
Sensitivity=0;
Specificity=0;
Gmean=0;
Fscore=0;
...
%% set parameters
 NumberofHiddenNeurons = 260;
npar  = size_Features* NumberofHiddenNeurons+NumberofHiddenNeurons ;          % number of optimization variables
varLo = -1;         % Lower  band of parameter
varHi =  1;         % Higher band of parameter
...
while ( (iteration <= maxIter) && (-maxProfit > accuracy) )
    ...
    for cuckooNumber = 1:numCuckooS
        temp=[cuckooPop{cuckooNumber}.center ; cuckooPop{cuckooNumber}.newPosition4Egg  ];
        temmProfit=[];
        for j=1:size(temp,1)
            temmProfit(j,1) = COST_elm( temp(j,:) ,1, d_train, d_test);
        end
        cuckooPop{cuckooNumber}.profitValues=temmProfit;
    end
    ...
    currentMaxProfit= COST_elm(currentBestCuckoo,1, d_train, d_test);
    if currentMaxProfit > globalMaxProfit
        globalBestCuckoo = currentBestCuckoo;
        globalMaxProfit = currentMaxProfit;
    end
    
    % Update cost maximization plot
    plot(iteration,1- globalMaxProfit,'ks','linewidth',2,'MarkerEdgeColor','k','MarkerFaceColor','g','MarkerSize',10)
   ...
   currentMaxProfit = COST_elm(currentBestCuckoo,1, d_train, d_test);
    ...
    cuckooPop{end}.profitValues= COST_elm(cuckooPop{end}.center,1, d_train, d_test);

    tmp = rand(1,npar).*globalBestCuckoo;
    tmp(find( tmp>varHi )) = varHi;
    tmp(find( tmp<varLo )) = varLo;
    cuckooPop{end-1}.center = tmp;
   [ cuckooPop{end-1}.profitValues,TrainingTime,Sensitivity,Specificity,Gmean,Fscore] = COST_elm(cuckooPop{end-1}.center,1, d_train, d_test);
...
disp('Accuracy training = ')
disp(globalMaxProfit)

testAccuracy=CalculateTestAccuracy(globalBestCuckoo,1, d_train, d_test)
...
figure
plot(1-costVector,'-ks','linewidth',3,'MarkerEdgeColor','k','MarkerFaceColor','g','MarkerSize',15)
xlabel 'Cuckoo Iteration'
ylabel 'Cost 1Value'
